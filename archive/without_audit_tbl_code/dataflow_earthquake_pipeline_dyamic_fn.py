...
# Define the complete schema for the JSON data structure
...
class ExtractDataFormAPI(beam.DoFn):
    """
      DoFn class to fetch JSON data from an API and yield it as a Python dictionary.

      - process(ele, api_url): Makes a GET request to `api_url`. If successful (status code 200),
        logs success and yields the JSON data as a dictionary; otherwise, logs an error and yields None.

      Parameters:
          ele: Unused placeholder for Beam's requirements.
          api_url (str): API Url to fetch data.

      Returns:
          dict: JSON data as a dictionary on success.
...
class ExtractRequiredDataAndFlatten(beam.DoFn):
    """
        Extracts and flattens required data from the nested dictionary (all data) structure.

        - process(all_data_dic):  Takes a dictionary with a 'features' key, iterates through each feature,
      and restructures the 'geometry' coordinates for each entry to add longitude, latitude, and depth.

        Parameters:
            all_data_dic (dict): data in dictionary format containing nested 'features'.

        Yields:
            dict: Flattened feature data with required fields and restructured 'geometry'.
        """
    def process(self, all_data_dic):
        import json
        extract_feature_lst = all_data_dic['features']  ## fetch feature lst from dic
...
class ApplyTransformation(beam.DoFn):
    """
        Applies data transformation to extract, convert, and restructure earthquake event details.

        - Converts UNIX timestamps to readable UTC dates.
        - Adds a new 'area' field based on the 'place' field.
        - Adds a new 'insert_date' field with the current UTC timestamp.
        - Prepares a final dictionary with all required fields for BigQuery.

        Parameters:
            properties_dic (dict): Dictionary containing earthquake event properties.
...
if __name__ == '__main__':
    ...
    # os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r"D:\Mohini Data Science\earthquake_ingestion_dataflow_dev\spark-learning-43150-ec7bab40e259.json"
    ...
    user_runner = arg.runner #user_runner = 'DataflowRunner'  # or 'DirectRunner', depending on needs
    api_url=arg.api_url
    pipeline_nm=arg.pipeline_nm

    ## Create PipelineOptions and set Google Cloud configurations.
    option_obj = PipelineOptions()
    google_cloud = option_obj.view_as(GoogleCloudOptions)
    option_obj.view_as(StandardOptions).runner = user_runner
    ...
    ## Set up logging for the pipeline execution.
    logging.basicConfig(level=logging.INFO)

    ## Get the current timestamp ('YYYYMMDD_HHMMSS' format)for unique file naming.
    ...
    ##  Second pipeline: Read from landing location, extract required data,flatten it and apply transformations, and write to silver layer and BigQuery.
    with beam.Pipeline(options=option_obj) as p2:
        read_data_from_landing_loc_apply_trans = (
                    p2 | 'Read data From GCS landing location' >> beam.io.ReadFromParquet(read_data_loc) ##list of dictionaries (key-value pairs) or a PCollection of rows
        ...
        ## Write cleaned data to GCS silver layer.

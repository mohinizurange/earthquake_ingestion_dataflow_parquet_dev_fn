...
class ExtractRequiredDataAndFlatten(beam.DoFn):
    def process(self, json_str):
        ...
        # print(type(data)) #str
        data_dic = json.loads(
            json_str)  ## Convert the string to JSON (Python dictionary) ## print(data_json,type(data_json)) ## dic
...
## define pardo class for add insert date cloumn
class AddInsertDateCol(beam.DoFn):

    def process(self,earthquake_dic ):
        from datetime import datetime
        ...
        earthquake_dic["insert_date"] = datetime.utcfromtimestamp(insert_date).strftime('%Y-%m-%d %H:%M:%S')
        yield earthquake_dic
...
if __name__ == '__main__':
    ...
    with beam.Pipeline(options=option_obj) as p:
        extract_data_api_and_write_gcs = (p | "StartPipeline" >> beam.Create([None])  # Initialize with dummy element
                                          ...
                                          | 'extracted data write to GCS' >> beam.io.WriteToText(landing_location,
    ...
    with beam.Pipeline(options=option_obj) as p2:
        read_data_from_landing_loc_apply_trans = (p2| 'Read data From GCS landing location' >> beam.io.ReadFromText(read_data_loc)
        ...
        write_clean_data_into_bq_loc = (read_data_from_landing_loc_apply_trans
                                |" add insert date " >> beam.ParDo(AddInsertDateCol())
